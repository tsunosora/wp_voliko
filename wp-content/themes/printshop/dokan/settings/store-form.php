...
<?php do_action( 'dokan_settings_before_form', $current_user, $profile_info ); ?>
    ...
    <form method="post" id="store-form"  action="">
        ...
        <?php if ( ! empty( $dokan_appearance ) && 'layout3' !== $dokan_appearance ): ?>
            <div class="dokan-banner">
                ...
                <div class="button-area<?php echo $banner ? ' dokan-hide' : ''; ?>">
                    ...
                    <a href="#" class="dokan-banner-drag dokan-btn dokan-btn-info dokan-theme"><?php _e( 'Upload banner', 'printshop' ); ?></a>
                    <p class="help-block">
                        <?php
                        /**
                        ...
                        $help_text = sprintf(
                            __('Upload a banner for your store. Banner size is (%sx%s) pixels.', 'printshop' ),
        ...
        <div class="row form-group">
            <div class="col-md-3">
                <label><?php _e( 'Profile Picture', 'printshop' ); ?></label>
            </div>
            <div class="col-md-9">
                <div class="dokan-w5 dokan-gravatar">
                    ...
                    <div class="gravatar-button-area<?php echo $gravatar ? ' dokan-hide' : ''; ?>">
                        <a href="#" class="dokan-pro-gravatar-drag dokan-btn dokan-btn-default"><i class="fa fa-cloud-upload"></i> <?php _e( 'Upload Photo', 'printshop' ); ?></a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row form-group">
            <div class="col-md-3">
                <label><?php _e( 'Store Name', 'printshop' ); ?></label>
            </div>
            <div class="col-md-9">
                <input id="dokan_store_name" required value="<?php echo $storename; ?>" name="dokan_store_name" placeholder="<?php _e( 'store name', 'printshop'); ?>" class="dokan-form-control" type="text">
            </div>
        </div>

        <div class="row form-group">
            <div class="col-md-3">
                <label><?php _e( 'Store Product Per Page', 'printshop' ); ?></label>
            </div>
            <div class="col-md-9">
        ...
        <div class="row form-group">
            <div class="col-md-3">
                <label><?php _e( 'Phone No', 'printshop' ); ?></label>
            </div>

            <div class="col-md-9">
                <input id="setting_phone" value="<?php echo $phone; ?>" name="setting_phone" placeholder="<?php _e( '+123456..', 'printshop' ); ?>" class="dokan-form-control input-md" type="text">
            </div>
        </div>
        ...
        <?php if ( ! is_int( key( $is_enable_op_discount ) ) && array_key_exists("order-discount", $is_enable_op_discount ) == "order-discount" ) : ?>
            <div class="dokan-form-group">
                <label><?php _e( 'Discount ', 'printshop' ); ?></label>
                <div class="dokan-w5 dokan-text-left">
                    <div class="checkbox">
                        <label class="dokan-control-label" for="lbl_setting_minimum_quantity">
                            ...
                            <?php _e( 'Enable storewide discount', 'printshop' ); ?>
                        </label>
                    </div>
                    <div class="show_if_needs_sw_discount <?php echo ($is_enable_order_discount=='yes') ? '' : 'hide_if_order_discount' ;?>">
                        <div class="dokan-text-left dokan-form-group">
                            <input id="setting_minimum_order_amount" value="<?php echo $setting_minimum_order_amount; ?>" name="setting_minimum_order_amount" placeholder="<?php _e( 'Minimum Order Amount', 'printshop' ); ?>" class="dokan-form-control input-md" type="number">
                        </div>
                        <div class="dokan-text-left dokan-form-group">
                            <input id="setting_order_percentage" value="<?php echo $setting_order_percentage; ?>" name="setting_order_percentage" placeholder="<?php _e( 'Percentage', 'printshop' ); ?>" class="dokan-form-control input-md" type="number" min="1" max="100">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row form-group group-checkbox">
            <div class="col-md-3">
                <label><?php _e( 'Email', 'printshop' ); ?></label>
            </div>
            <div class="col-md-9">
                <div class="checkbox">
                    <label>
                        <input type="hidden" name="setting_show_email" value="no">
                        <input type="checkbox" name="setting_show_email" value="yes"<?php checked( $show_email, 'yes' ); ?>> <?php _e( 'Show email address in store', 'printshop' ); ?>
                    </label>
                </div>
            </div>
        </div>

        <div class="row form-group group-checkbox-last">
            <div class="col-md-3">
                <label><?php _e( 'More product', 'printshop' ); ?></label>
            </div>
            <div class="col-md-9">
                <div class="checkbox">
                    <label>
                        <input type="hidden" name="setting_show_more_ptab" value="no">
                        <input type="checkbox" name="setting_show_more_ptab" value="yes"<?php checked( $show_email, 'yes' ); ?>> <?php _e( 'Enable tab on product single page view', 'printshop' ); ?>
                    </label>
                </div>
            </div>
        </div>
        ...
        <div class="row form-group">
            <div class="col-md-3">
                <label><?php _e( 'Map', 'printshop' ); ?></label>
            </div>
            ...
            <div class="col-md-9">
                ...
                <div class="dokan-map-wrap">
                    <div class="dokan-map-search-bar">
                        <input id="dokan-map-add" type="text" class="dokan-map-search dokan-form-control" value="<?php echo $map_address; ?>" name="find_address" placeholder="<?php _e( 'Type an address to find', 'printshop' ); ?>" size="30" />
                        <a href="#" class="dokan-map-find-btn" id="dokan-location-find-btn" type="button"><?php _e( 'Find Address', 'printshop' ); ?></a>
        ...
        if ( $tnc_enable == 'on' ) :
            ?>
        <div class="row form-group group-checkbox-terms">
            <div class="col-md-3">
                <label><?php _e( 'Terms and Conditions', 'printshop' ); ?></label>
            </div>
            <div class="col-md-9">
                <div class="dokan-w5 dokan-text-left dokan_tock_check">
                    <div class="checkbox">
                        <label>
                            <input type="checkbox" id="dokan_store_tnc_enable" value="on" <?php echo $enable_tnc == 'on' ? 'checked':'' ; ?> name="dokan_store_tnc_enable" ><?php _e( 'Show terms and conditions in store page', 'printshop' ); ?>
                        </label>
                    </div>
                </div>
            </div>
        ...
        <div class="row form-group" id="dokan_tnc_text">
            <div class="col-md-3">
                <label><?php _e( 'TOC Details', 'printshop' ); ?></label>
            </div>
            <div class="col-md-9">
                <?php
                    $settings = array(
        ...
        <div class="dokan-form-group">
            ...
            <div class="dokan-w4 ajax_prev dokan-text-left">
                <input type="submit" name="dokan_update_store_settings" class="dokan-btn dokan-btn-danger dokan-btn-theme" value="<?php esc_attr_e( 'Update Settings', 'printshop' ); ?>">
            </div>
        </div>
    </form>
...
<script type="text/javascript">
    ...
    (function($) {
        var dokan_address_wrapper = $( '.dokan-address-fieldz' );
