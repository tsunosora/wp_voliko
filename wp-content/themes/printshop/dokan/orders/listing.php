...
if ( $user_orders ) {
    ...
    <table class="dokan-table dokan-table-striped">
        <thead>
            <tr>
                <th><?php _e( 'Order', 'printshop' ); ?></th>
                <th><?php _e( 'Order Total', 'printshop' ); ?></th>
                <th><?php _e( 'Status', 'printshop' ); ?></th>
                <th><?php _e( 'Customer', 'printshop' ); ?></th>
                <th><?php _e( 'Date', 'printshop' ); ?></th>
                <th width="17%"><?php _e( 'Action', 'printshop' ); ?></th>
            </tr>
        </thead>
        <tbody>
            <?php
            foreach ($user_orders as $order) {
                ...
                ?>
                <tr >
                    <td class="dokan-order-id" data-title="<?php _e( 'Order', 'printshop' ); ?>" >
                        <?php echo '<a href="' . wp_nonce_url( add_query_arg( array( 'order_id' => $the_order->get_id() ), dokan_get_navigation_url( 'orders' ) ), 'dokan_view_order' ) . '"><strong>' . sprintf( __( 'Order %s', 'printshop' ), esc_attr( $the_order->get_order_number() ) ) . '</strong></a>'; ?>
                    </td>
                    <td class="dokan-order-total" data-title="<?php _e( 'Order Total', 'printshop' ); ?>" >
                        <?php echo $the_order->get_formatted_order_total(); ?>
                    </td>
                    <td class="dokan-order-status" data-title="<?php _e( 'Status', 'printshop' ); ?>" >
                    ...
                    <td class="dokan-order-customer" data-title="<?php _e( 'Customer', 'printshop' ); ?>" >
                        <?php
                        ...
                        if ( !empty( $user_info ) ) {
                            ...
                            $user = __( 'Guest', 'printshop' );
                        }

                        echo $user;
                        ?>
                    </td>
                    <td class="dokan-order-date" data-title="<?php _e( 'Date', 'printshop' ); ?>" >
                        <?php
                        if ( '0000-00-00 00:00:00' == $the_order->order_date ) {
                            $t_time = $h_time = __( 'Unpublished', 'printshop' );
                        } else {
                            $t_time = get_the_time( __( 'Y/m/d g:i:s A', 'printshop' ), $the_order->get_id() );

                            $gmt_time = strtotime( $the_order->order_date . ' UTC' );
                            $time_diff = current_time( 'timestamp', 1 ) - $gmt_time;

                            if ( $time_diff > 0 && $time_diff < 24 * 60 * 60 )
                                $h_time = sprintf( __( '%s ago', 'printshop' ), human_time_diff( $gmt_time, current_time( 'timestamp', 1 ) ) );
                            else
                                $h_time = get_the_time( __( 'Y/m/d', 'printshop' ), $the_order->get_id() );
                        }

                        echo '<abbr title="' . esc_attr( $t_time ) . '">' . esc_html( apply_filters( 'post_date_column_time', $h_time, $the_order->get_id() ) ) . '</abbr>';
                        ?>
                    </td>
                    <td class="dokan-order-action" width="17%" data-title="<?php _e( 'Action', 'printshop' ); ?>" >
                        <?php
...
<?php } else { ?>
    ...
    <div class="dokan-error">
        <?php _e( 'No orders found', 'printshop' ); ?>
    </div>
