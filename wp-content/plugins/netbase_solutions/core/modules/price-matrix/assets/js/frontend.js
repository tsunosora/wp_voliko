	...
	var pm_frontend = {
		...
		init: function() {
			...
			$(window).load(function() {
				setTimeout(function(){
					pm_frontend.removeMissingAttribute();
				}, 1000);
			});
		},
		
		removeMissingAttribute: function() {
			$( ".has-price-matrix .variations select" ).each(function( index ) {
				var $attribute_id = $(this).attr('id');
				console.log($attribute_id);
				$(this).find('option').each(function( index ) {
					var $val = $(this).attr('value');
					$('table.un-variations select#' + $attribute_id + ' option[value="' + $val + '"]').addClass('show');
				});
			});

			$( "table.un-variations select" ).each(function( index ) {
				$(this).find('option:not(.show)').remove();
			});
		},
