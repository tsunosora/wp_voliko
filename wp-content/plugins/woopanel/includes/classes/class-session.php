...
/**
 * WooDashboard_Session Class
 *
 * @since 1.5
 */
class WooDashboard_Session {
	...
	public function __construct() {
		...
		if( $this->use_php_sessions ) {
			...
			if ( ! class_exists( 'Recursive_ArrayAccess' ) ) {
				require_once WOODASHBOARD_INC_DIR . 'classes/class-recursive-arrayaccess.php';
			}

	 		if ( ! class_exists( 'WP_Session' ) ) {
				require_once WOODASHBOARD_INC_DIR . 'classes/class-wp-session.php';
				require_once WOODASHBOARD_INC_DIR . 'helper/session.php';
	 		}
	...
	public function init() {
		...
		if( $this->use_php_sessions ) {
			$key = sprintf( 'woopanel%s', $this->prefix );
	...
	public function set( $key, $value ) {
		...
		if( $this->use_php_sessions ) {
			...
			$_SESSION[ sprintf( 'woopanel%s', $this->prefix ) ] = $this->session;
		}
	...
	public function get_blacklist() {
		...
		if( ! empty( $folder ) ) {
			foreach( $blacklist as $path ) {
				$blacklist[] = sprintf( '%s/%s', $folder, $path );
			}
		}
