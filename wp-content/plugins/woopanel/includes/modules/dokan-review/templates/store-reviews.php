        ...
        <div class="dokan-widget-area widget-collapse">
            ...
            if ( ! dynamic_sidebar( 'sidebar-store' ) ) {
                ...
                if ( class_exists( 'Dokan_Store_Location' ) ) {
                    the_widget( 'Dokan_Store_Category_Menu', array( 'title' => esc_html__( 'Store Category', 'woopanel' ) ), $args );

                    if ( dokan_get_option( 'store_map', 'dokan_general', 'on' ) == 'on' ) {
                        the_widget( 'Dokan_Store_Location', array( 'title' => esc_html__( 'Store Location', 'woopanel' ) ), $args );
                    }

                    if ( dokan_get_option( 'store_open_close', 'dokan_general', 'on' ) == 'on' ) {
                        the_widget( 'Dokan_Store_Open_Close', array( 'title' => esc_html__( 'Store Time', 'woopanel' ) ), $args );
                    }

                    if( dokan_get_option( 'contact_seller', 'dokan_general', 'on' ) == 'on' ) {
                        the_widget( 'Dokan_Store_Contact_Form', array( 'title' => esc_html__( 'Contact Vendor', 'woopanel' ) ), $args );
                    }
                }
            }
            ?>
...
<div id="dokan-primary" class="dokan-single-store dokan-w8">
    <div id="dokan-content" class="store-review-wrap woocommerce" role="main">
        ...
        <?php
        $dokan_template_reviews = NB_Dokan_Review::init();
        $id                     = $store_user->ID;
        $post_type              = 'product';
        $limit                  = 20;
        $status                 = '1';
        ...
        <div id="reviews">
            <div id="comments">
              ...
              <?php do_action( 'dokan_review_tab_before_comments' ); ?>
                ...
                <h2 class="headline"><?php esc_html_e( 'Vendor Review', 'woopanel' ); ?></h2>

                <ol class="commentlist">
                    <?php print( $dokan_template_reviews->render_store_tab_comment_list( $comments , $store_user->ID) ); ?>
                </ol>
        ...
        <?php
        print( $dokan_template_reviews->review_pagination( $id, $post_type, $limit, $status ) );
        ?>
